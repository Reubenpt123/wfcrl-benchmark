...
class Args:
    ...
    """whether to save model into the `~/code/wfcrl-benchmark/runs/{run_name}` folder"""
    freq_eval: int = 50
    """Number of iterations between eval"""
    wind_data: str = "~/code/wfcrl-benchmark/data/smarteole.csv"
...
if __name__ == "__main__":
    ...
    if args.track:
        ...
        wandb.tensorboard.patch(root_logdir=f"~/code/wfcrl-benchmark/runs/{run_name}", pytorch=False, tensorboard_x=False, save=False)
        wandb.init(
    ...
    writer = LocalSummaryWriter(f"~/code/wfcrl-benchmark/runs/{run_name}", )
    writer.add_config(vars(args))
    model_path = f"~/code/wfcrl-benchmark/runs/{run_name}/{args.exp_name}.cleanrl_model"
    # TRY NOT TO MODIFY
    ...
    fig.savefig(f"~/code/wfcrl-benchmark/runs/{run_name}/plot.png")

    # Save the run name for batch scripts to find it
    with open("~/code/wfcrl-benchmark/scripts/most_recent_models/idqn_path.txt", "w") as file:
        file.write(f"~/code/wfcrl-benchmark/runs/{run_name}")
