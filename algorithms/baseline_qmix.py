...
class Args:
    ...
    """total training iterations of the experiments (was: total_timesteps)"""
...
if __name__ == "__main__":
    ...
    for training_step in range(args.total_iterations):
        ...
        if (episode_id > args.learning_starts) and last_done:
            ...
            if episode_id % 5 == 0:
                writer.add_scalar(f"losses/agent_{idagent}/td_loss", loss, training_step)
                writer.add_scalar(f"losses/agent_{idagent}/q_values", qmixer_old_val.mean().item(), training_step)
    ...
    most_recent_path = PROJECT_ROOT / "scripts" / "most_recent_models" / "qmix_path.txt"
